...
def get_begin_point(final_output_path):
    params_list = glob.glob(final_output_path + '/*.params')
    iter_nums = [int(i[:-7].split('-')[1]) for i in params_list]
    epoch_nums = [i for i in iter_nums if 0 < i < 3]
    batch_nums = [i for i in iter_nums if 1000 < i]

    return max(epoch_nums) if len(epoch_nums) > 0 else 0 , max(batch_nums) if len(batch_nums) > 0 else 0

def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    mx.random.seed(3)
    np.random.seed(3)
    logger, final_output_path, tb_log_path = create_logger(config.output_path, config.log_path, args.cfg, config.dataset.image_set)
    ...
    if config.TRAIN.RESUME:
        begin_epoch, begin_batch = get_begin_point(final_output_path)
        print('continue training from ', begin_epoch, begin_batch)
        arg_params, aux_params = load_param(prefix, begin_batch, convert=True)
    else:
    ...
    if config.TRAIN.RESUME:
        mod._preload_opt_states = '%s-%04d.states'%(prefix, begin_batch)
    ...
    # callback
    tensorboard_callback = callback.TensorboardCallback(tb_log_path) if config.use_tensorboard else None
    batch_end_callback = [callback.Speedometer(train_data.batch_size, frequent=args.frequent)]
    if tensorboard_callback:
        batch_end_callback.append(tensorboard_callback)
    ...
    mod.fit(train_data, eval_metric=eval_metrics, epoch_end_callback=epoch_end_callback,
            ...
            arg_params=arg_params, aux_params=aux_params, begin_epoch=begin_epoch, num_epoch=end_epoch, begin_batch = begin_batch)
def main():
