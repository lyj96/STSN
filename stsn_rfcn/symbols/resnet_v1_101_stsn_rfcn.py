...
class resnet_v1_101_stsn_rfcn(Symbol):
    ...
    def get_resnet_v1(self, data):
        ...
        #def
        res4b20_branch2b_offset = mx.symbol.Convolution(name='res4b20_branch2b_offset', data = res4b20_branch2a_relu,
                                                      num_filter=72, pad=(1, 1), kernel=(3, 3), stride=(1, 1), dilate=(1, 1), cudnn_off=True)
        res4b20_branch2b = mx.contrib.symbol.DeformableConvolution(name='res4b20_branch2b', data=res4b20_branch2a_relu, offset=res4b20_branch2b_offset,
                                                                 num_filter=256, pad=(1, 1), kernel=(3, 3), num_deformable_group=4,
        ...
        #def
        res4b21_branch2b_offset = mx.symbol.Convolution(name='res4b21_branch2b_offset', data = res4b21_branch2a_relu,
                                                      num_filter=72, pad=(1, 1), kernel=(3, 3), stride=(1, 1), dilate=(1, 1), cudnn_off=True)
        res4b21_branch2b = mx.contrib.symbol.DeformableConvolution(name='res4b21_branch2b', data=res4b21_branch2a_relu, offset=res4b21_branch2b_offset,
                                                                 num_filter=256, pad=(1, 1), kernel=(3, 3), num_deformable_group=4,
                                                                 stride=(1, 1), dilate=(1, 1), no_bias=True)
    ...
    def get_aligned_feat(self, refer_feat, sup_feat):
        concat_feat = mx.symbol.Concat(refer_feat, sup_feat, dim=1)

        # concat_feat = mx.symbol.Convolution(name='concat_feat', data=concat_feat, num_filter=1024, pad=(1, 1),
        #                                 kernel=(3, 3), stride=(1, 1), no_bias=False)
        # concat_feat = mx.symbol.Activation(name='concat_feat_ReLU1', data=concat_feat, act_type='relu')

        aligned_feat_1_offset = mx.symbol.Convolution(name='aligned_feat_1_offset', data = concat_feat,
                                                      num_filter=72, pad=(1, 1), kernel=(3, 3), stride=(1, 1), cudnn_off=True)
        aligned_feat_1 = mx.contrib.symbol.DeformableConvolution(name='aligned_feat_1', data=concat_feat, offset=aligned_feat_1_offset,
                                                                 num_filter=1024, pad=(1, 1), kernel=(3, 3), num_deformable_group=4,
                                                                 stride=(1, 1), no_bias=True)
        aligned_feat_2_offset = mx.symbol.Convolution(name='aligned_feat_2_offset', data = aligned_feat_1,
                                                      num_filter=72, pad=(1, 1), kernel=(3, 3), stride=(1, 1), cudnn_off=True)
        aligned_feat_2 = mx.contrib.symbol.DeformableConvolution(name='aligned_feat_2', data=aligned_feat_1, offset=aligned_feat_2_offset,
                                                                 num_filter=1024, pad=(1, 1), kernel=(3, 3), num_deformable_group=4,
                                                                 stride=(1, 1), no_bias=True)
        aligned_feat_3_offset = mx.symbol.Convolution(name='aligned_feat_3_offset', data = aligned_feat_2,
                                                      num_filter=72, pad=(1, 1), kernel=(3, 3), stride=(1, 1),cudnn_off=True)
        aligned_feat_3 = mx.contrib.symbol.DeformableConvolution(name='aligned_feat_3', data=aligned_feat_2, offset=aligned_feat_3_offset,
                                                                 num_filter=1024, pad=(1, 1), kernel=(3, 3), num_deformable_group=4,
                                                                 stride=(1, 1),no_bias=True)
        aligned_feat_4_offset = mx.symbol.Convolution(name='aligned_feat_4_offset', data = aligned_feat_3,
                                                      num_filter=72, pad=(1, 1), kernel=(3, 3), stride=(1, 1), cudnn_off=True)
        aligned_feat = mx.contrib.symbol.DeformableConvolution(name='aligned_feat', data=sup_feat, offset=aligned_feat_4_offset,
                                                                 num_filter=1024, pad=(1, 1), kernel=(3, 3), num_deformable_group=4,
                                                                 stride=(1, 1), no_bias=True)
        return aligned_feat
    ...
    def get_train_symbol(self, cfg):
        ...
        concat_feat = mx.symbol.Concat(conv_feat[1], conv_feat[2], dim=0)
        feat_copies = mx.sym.tile(conv_feat[0], reps=(2, 1, 1, 1))

        aligned_feat = self.get_aligned_feat(feat_copies, concat_feat)
        aligned_feat = mx.sym.SliceChannel(aligned_feat, axis=0, num_outputs=2)

        # warping
        aligned_feat_bef = aligned_feat[0]
        aligned_feat_aft = aligned_feat[1]

        # pass through EmbedNet
        concat_embed_data = mx.symbol.Concat(*[conv_feat[0], aligned_feat_bef, aligned_feat_aft], dim=0)
        ...
        select_conv_feat = weight1 * aligned_feat_bef + weight2 * aligned_feat_aft
        #select_conv_feat = conv_feat[0] + conv_feat[1] +conv_feat[2]
    ...
    def get_feat_symbol(self, cfg):
        ...
        group = mx.sym.Group([conv_feat, im_info, feat_cache])
        self.sym = group
        return group
    ...
    def get_aggregation_symbol(self, cfg):
        ...
        cur_feat = mx.symbol.slice_axis(feat_cache, axis=0, begin=cfg.TEST.KEY_FRAME_INTERVAL,
                                         end=cfg.TEST.KEY_FRAME_INTERVAL + 1)
        cur_feat = mx.sym.tile(cur_feat, reps=(data_range, 1, 1, 1))

        aligned_feat = self.get_aligned_feat(cur_feat, feat_cache)
        embed_output = self.get_embednet(aligned_feat)
        ...
        warp_list = mx.sym.SliceChannel(aligned_feat, axis=0, num_outputs=data_range)
    ...
    def init_weight_rfcn(self, cfg, arg_params, aux_params):
        arg_params['res4b20_branch2b_offset_weight'] = mx.nd.zeros(shape=self.arg_shape_dict['res4b20_branch2b_offset_weight'])
        arg_params['res4b20_branch2b_offset_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['res4b20_branch2b_offset_bias'])
        arg_params['res4b21_branch2b_offset_weight'] = mx.nd.zeros(shape=self.arg_shape_dict['res4b21_branch2b_offset_weight'])
        arg_params['res4b21_branch2b_offset_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['res4b21_branch2b_offset_bias'])
    ...
    def init_aligned_weight(self, cfg, arg_params, aux_params):
        arg_params['aligned_feat_1_offset_weight'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_1_offset_weight'])
        arg_params['aligned_feat_1_offset_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_1_offset_bias'])
        arg_params['aligned_feat_2_offset_weight'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_2_offset_weight'])
        arg_params['aligned_feat_2_offset_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_2_offset_bias'])
        arg_params['aligned_feat_3_offset_weight'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_3_offset_weight'])
        arg_params['aligned_feat_3_offset_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_3_offset_bias'])
        arg_params['aligned_feat_4_offset_weight'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_4_offset_weight'])
        arg_params['aligned_feat_4_offset_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['aligned_feat_4_offset_bias'])

        arg_params['aligned_feat_1_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['aligned_feat_1_weight'])
        arg_params['aligned_feat_2_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['aligned_feat_2_weight'])
        arg_params['aligned_feat_3_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['aligned_feat_3_weight'])
        arg_params['aligned_feat_weight'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['aligned_feat_weight'])
    ...
    def init_weight(self, cfg, arg_params, aux_params):
        ...
        self.init_aligned_weight(cfg, arg_params, aux_params)